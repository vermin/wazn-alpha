...
Core tests take longer than any other WAZN tests, due to the high amount of computational work involved in validating core components.
...
To run only WAZN's core tests (after building):
...
To run only WAZN's crypto tests (after building):
...
```
waznd --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `wazn-wallet-rpc` with RPC port 11789. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only WAZN's hash tests (after building):
...
To run only WAZN's performance tests (after building):
...
To run only WAZN's unit tests (after building):
